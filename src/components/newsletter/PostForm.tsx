      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="postTitle" placeholder="Título da publicação" value={postTitle} onChange={e => setPostTitle(e.target.value)} className="bg-inventu-darker/80 border-white/10 text-white" required />
          </div>
          
          <div className="space-y-2">
            <Label htmlFor="content" className="text-white">Conteúdo</Label>
            <Textarea id="content" placeholder="O que você deseja compartilhar?" value={content} onChange={e => setContent(e.target.value)} className="min-h-[120px] bg-inventu-darker/80 border-white/10 resize-none text-white" required />
          </div>
          ...
          {mediaType !== 'none' && <div className="space-y-2">
              <input type="file" ref={fileInputRef} className="hidden" accept={mediaType === 'image' ? 'image/*' : 'video/*'} onChange={handleFileChange} />
              
              {!mediaUrl ? <Button type="button" variant="outline" onClick={handleFileSelect} className="w-full h-20 border-dashed border-white/20 bg-inventu-darker/50 text-white/70">
                  <Upload className="mr-2 h-4 w-4" />
                  Selecionar {mediaType === 'image' ? 'imagem' : 'vídeo'}
                </Button> : <div className="relative rounded-md overflow-hidden">
                  <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2 z-10 h-8 w-8 rounded-full" onClick={handleRemoveMedia}>
                    <X className="h-4 w-4" />
                  </Button>
                  
                  {mediaType === 'image' ? <AspectRatio ratio={4 / 5} className="bg-inventu-darker">
                      <img src={mediaUrl} alt="Preview" className="w-full h-full object-cover" />
                    </AspectRatio> : <AspectRatio ratio={4 / 5} className="bg-inventu-darker">
        ...
        <CardFooter>
          <Button type="submit" className="w-full bg-inventu-blue hover:bg-inventu-blue/80 text-white" disabled={isSubmitting || isUploading || mediaType !== 'none' && !mediaUrl}>
