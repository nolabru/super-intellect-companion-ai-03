...
const GalleryList: React.FC<GalleryListProps> = ({
  ...
  return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 p-3 sm:p-4 px-0 py-0">
      {media.map(item => <GalleryMediaCard key={item.id} item={item} onDelete={onDeleteItem} />)}
    </div>;
};
