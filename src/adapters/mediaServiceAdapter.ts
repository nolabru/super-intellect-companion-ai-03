...
export function useMediaServiceAdapter(options: MediaServiceOptions = { service: 'auto' }) {
      ...
      return new Promise((resolve, reject) => {
        const pollInterval = setInterval(async () => {
          try {
            ...
            if (result.status === 'finished' && result.mediaUrl) {
  ...
  // Register audio processor for audio generation
  taskManager.registerTaskProcessor('audio', async (task: Task) => {
    try {
      // Default implementation for audio
      return {
        ...task,
        status: 'failed',
        progress: 0,
      ...
      console.error('[mediaServiceAdapter] Error in audio task processor:', error);
      return {
        ...task,
        status: 'failed',
        progress: 0,
