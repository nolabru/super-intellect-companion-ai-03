...
serve(async (req) => {
  ...
  try {
    ...
    const { spreadsheetId, range, values, userId } = await req.json()
    ...
    if (!spreadsheetId || !range || !values) {
      throw new Error('Parâmetros incompletos: spreadsheetId, range e values são obrigatórios')
    }
    ...
    if (!sheetsResponse.ok) {
      ...
      throw new Error(`Erro ao escrever na planilha: ${sheetsResponse.status}`)
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        updatedRange: sheetsData.updatedRange,
        updatedRows: sheetsData.updatedRows,
        spreadsheetUrl,
      }),
      {
