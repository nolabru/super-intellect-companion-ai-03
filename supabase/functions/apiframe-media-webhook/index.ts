...
serve(async (req) => {
  ...
  try {
    console.log("[apiframe-media-webhook] Recebida notificação de webhook da APIframe");
    ...
    try {
      ...
      console.error("[apiframe-media-webhook] Erro ao analisar corpo da requisição:", parseError);
      return new Response(
    ...
    console.log("[apiframe-media-webhook] Payload recebido:", {
    ...
    if (!payload.task_id) {
      console.error("[apiframe-media-webhook] ID da tarefa ausente no payload do webhook");
      return new Response(
    ...
    if (taskError) {
      console.error(`[apiframe-media-webhook] Erro ao buscar tarefa ${payload.task_id}:`, taskError);
      return new Response(
    ...
    console.log(`[apiframe-media-webhook] Tarefa encontrada no banco de dados:`, {
      id: taskData.task_id,
    ...
    console.log(`[apiframe-media-webhook] Status da tarefa atualizado: ${status}, URL da mídia: ${mediaUrl || 'não disponível'}`);
    ...
    if (updateError) {
      console.error(`[apiframe-media-webhook] Erro ao atualizar tarefa no banco de dados:`, updateError);
      return new Response(
    ...
    console.log(`[apiframe-media-webhook] Registro da tarefa atualizado com sucesso`);

    // Se a tarefa foi completada com sucesso e temos URL de mídia, criar evento de mídia pronta
    if (status === 'completed' && mediaUrl) {
      console.log(`[apiframe-media-webhook] Criando evento de mídia pronta para tarefa ${payload.task_id}`);
      ...
      if (eventError) {
        console.error(`[apiframe-media-webhook] Erro ao inserir evento de mídia pronta:`, eventError);
      } else {
        console.log(`[apiframe-media-webhook] Evento de mídia pronta criado com sucesso`);
      }
    }
    ...
    console.error("[apiframe-media-webhook] Erro ao processar webhook:", error);
    return new Response(
      JSON.stringify({ 
