...
serve(async (req) => {
  ...
  try {
    ...
    if (!APIFRAME_API_KEY) {
      console.error("[apiframe-task-status] APIFRAME_API_KEY não configurada");
    ...
    try {
      ...
      console.error("[apiframe-task-status] Erro ao analisar corpo da requisição:", parseError);
      return new Response(
    ...
    if (!taskId) {
      console.error("[apiframe-task-status] taskId é obrigatório");
      return new Response(
    ...
    console.log(`[apiframe-task-status] Verificando status da tarefa: ${taskId}`);
    ...
    if (taskError) {
      console.error(`[apiframe-task-status] Erro ao buscar tarefa no banco de dados:`, taskError);
    } else if (taskData) {
      console.log(`[apiframe-task-status] Encontrado registro de tarefa no banco de dados:`, {
        id: taskData.task_id,
      ...
      if (taskData.status === 'completed' || taskData.status === 'failed') {
        console.log(`[apiframe-task-status] Retornando status da tarefa a partir do banco de dados`);
        return new Response(
          JSON.stringify({
    ...
    console.log(`[apiframe-task-status] Consultando status na API: ${APIFRAME_API_BASE_URL}/task/${taskId}`);
    
    const response = await fetch(`${APIFRAME_API_BASE_URL}/task/${taskId}`, {
      method: "GET",
      headers: {
    ...
    console.log(`[apiframe-task-status] Status da resposta da API: ${response.status}`);
    
    // Ler o corpo da resposta como texto para análise
    const responseText = await response.text();
    console.log(`[apiframe-task-status] Resposta bruta da API: ${responseText.substring(0, 200)}...`);

    // Verificar se a resposta foi bem-sucedida
    if (!response.ok) {
      console.error(`[apiframe-task-status] Erro na resposta da API:`, responseText);
      
      let errorMessage;
      try {
        const errorData = JSON.parse(responseText);
        errorMessage = errorData.error?.message || `Error from APIframe: ${response.statusText}`;
      } catch (e) {
        errorMessage = `Error from APIframe: ${response.statusText}`;
      }
    ...
    try {
      ...
      console.error(`[apiframe-task-status] Falha ao analisar resposta JSON:`, e);
      throw new Error(`Failed to parse API response from APIframe`);
    }

    console.log(`[apiframe-task-status] Resposta analisada com sucesso:`, {
    ...
    console.log(`[apiframe-task-status] Status mapeado: ${status}, URL da mídia: ${mediaUrl || 'não disponível'}`);
    
    // Atualizar o registro no banco de dados com as informações mais recentes
    if (status === 'completed' || status === 'failed') {
      console.log(`[apiframe-task-status] Atualizando registro da tarefa no banco de dados`);
      ...
      if (updateError) {
        console.error(`[apiframe-task-status] Erro ao atualizar tarefa no banco de dados:`, updateError);
      }
      ...
      if (status === 'completed' && mediaUrl) {
        ...
        console.log(`[apiframe-task-status] Criando evento de mídia pronta para tarefa ${taskId}`);
        ...
        if (eventError) {
          console.error(`[apiframe-task-status] Erro ao inserir evento de mídia pronta:`, eventError);
        } else {
          console.log(`[apiframe-task-status] Evento de mídia pronta criado com sucesso`);
        }
      }
    }
    ...
    console.error("[apiframe-task-status] Erro:", error);
    return new Response(
      JSON.stringify({ 
